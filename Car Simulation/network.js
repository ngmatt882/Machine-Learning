class NerualNetwork{
    // accepts an array of int to determine how many neurons each level will have
    constructor(neuronCounts){
        this.levels = [];
        for(let i = 0; i < neuronCounts.length - 1; ++i){
            this.levels.push(new Level(neuronCounts[i], neuronCounts[i+1]));
        }
    }

    static mutateNetwork(network, mutationRate){
        for(let i = 0; i < network.levels.length; ++i){
            Level.mutate(network.levels[i], mutationRate);
        }
    }

    static feedForward(inputs, network){
        let outputs = Level.feedForward(inputs, network.levels[0]);
        for(let i = 1; i < network.levels.length; ++i){
            outputs = Level.feedForward(outputs, network.levels[i]);
        }

        return outputs;
    }
}

class Level{
    constructor(numInputs, numOutputs){
        this.inputs = new Array(numInputs);
        this.outputs = new Array(numOutputs);
        this.biases = new Array(numOutputs);

        this.weights = [];
        for(let i = 0; i < this.inputs.length; ++i){
            this.weights[i] = new Array(numOutputs);
        }

        Level.#randomize(this);
    }

    static mutate(level, mutationRate){
        for(let i = 0; i < level.inputs.length; ++i){
            for(let j = 0; j < level.outputs.length; ++j){
                const m = Math.random();
                if(m < mutationRate){
                    level.weights[i][j] = Math.random()*2 - 1;
                }
            }
        }

        for(let i = 0; i < level.biases.length; ++i){
            const m = Math.random();
            if(m < mutationRate){
                level.biases[i] = Math.random()*2 - 1;
            }
        }
    }

    // randomizes the biases and weights
    static #randomize(level){
        for(let i = 0; i < level.inputs.length; ++i){
            for(let j = 0; j < level.outputs.length; ++j){
                level.weights[i][j] = Math.random()*2 - 1; 
            }
        }

        for(let i = 0; i < level.biases.length; ++i){
            level.biases[i] = Math.random()*2 - 1;
        }
    }

    static feedForward(inputs, level){
        // set inputs
        for(let i = 0; i < level.inputs.length; ++i){
            level.inputs[i] = inputs[i];
        }

        for(let i = 0; i < level.outputs.length; ++i){
            let sum = 0;
            for(let j = 0; j < level.inputs.length; ++j){
                sum += level.inputs[j] * level.weights[j][i];
            }

            if(sum > level.biases[i]){
                level.outputs[i] = 1;
            }
            else{
                level.outputs[i] = 0;
            }
        }
        
        return level.outputs;

    }
}